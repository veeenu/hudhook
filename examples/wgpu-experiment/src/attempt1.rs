...
use windows::Win32::Graphics::Gdi::{
    BeginPaint, CreateRectRgn, CreateSolidBrush, DeleteObject, DrawTextA, EndPaint, InvalidateRgn,
    Rectangle, RedrawWindow, SelectObject, UpdateWindow, DT_CENTER, DT_SINGLELINE, DT_VCENTER,
    HBRUSH, HDC, HRGN, PAINTSTRUCT, RDW_ALLCHILDREN, RDW_ERASE, RDW_FRAME, RDW_INTERNALPAINT,
    RDW_INVALIDATE,
};
...
unsafe fn create_overlay_window(target_hwnd: HWND) -> Result<(HWND, HINSTANCE)> {
    ...
    // SetLayeredWindowAttributes(hwnd, COLORREF(0x000000ff), 50,
    // LWA_COLORKEY).unwrap();
    // SetWindowLongW(hwnd, GWL_STYLE, (WS_VISIBLE | WS_POPUP).0 as i32);
