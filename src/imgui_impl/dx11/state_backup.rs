  ...
  pub fn backup(ctx: &ID3D11DeviceContext) -> StateBackup {
    ...
    unsafe {
      ctx.RSGetScissorRects(
        &mut r.scissor_rects_count, &mut r.scissor_rects as *mut _ as *mut _
      );
      ctx.RSGetViewports(
        &mut r.viewports_count, &mut r.viewports as *mut _ as *mut _
      );
      ctx.RSGetState(&mut r.rasterizer_state);
      ctx.OMGetBlendState(
        &mut r.blend_state, &mut r.blend_factor, &mut r.sample_mask
      );
  ...
  pub fn restore(self, ctx: &ID3D11DeviceContext) {
    unsafe {
      ctx.RSSetScissorRects(
        self.scissor_rects_count,
        &self.scissor_rects as *const _
      );
      ctx.RSSetViewports(self.viewports_count, &self.viewports as *const _);
      ctx.RSSetState(self.rasterizer_state);
      self.rasterizer_state.as_ref().map(|r| r.Release()).unwrap_or(0);
      ...
      ctx.OMSetDepthStencilState(
        self.depth_stencil_state, self.stencil_ref
      );
      self.depth_stencil_state.as_ref().map(|r| r.Release()).unwrap_or(0);
      ctx.PSSetShaderResources(0, 1, &self.ps_shader_resource);
      self.ps_shader_resource.as_ref().map(|r| r.Release()).unwrap_or(0);
      ...
      for i in 0..self.ps_instances_count {
        self.ps_instances[i as usize].as_ref().map(|r| r.Release()).unwrap_or(0);
      }
      ctx.VSSetShader(
        self.vertex_shader,
      ...
      self.vertex_shader.as_ref().map(|r| r.Release()).unwrap_or(0);
      for i in 0..self.vs_instances_count {
        self.vs_instances[i as usize].as_ref().map(|r| r.Release()).unwrap_or(0);
      }
      ...
      self.vertex_buffer.as_ref().map(|r| r.Release()).unwrap_or(0);
